...
#define DECLARE_ALLOCATOR(type, alloc_step)                                 \
    ...
    static inline type* type##_alloc() {                                    \
        ...
        if(_##type##_allocator_buffer->size == 0) {                         \
            /* bo back on the stack to see if there us any uused obj */     \
            _##type##_allocator_buffer = _##type##_allocator_buffer->prev;  \
            if(_##type##_allocator_buffer->size == 0) {                     \
                type* objs = malloc(alloc_step*sizeof(type));               \
                for(i = 0; i < alloc_step; ++i) {                           \
                    _##type##_allocator_buffer->objs[i] = objs + i;         \
                }                                                           \
                _##type##_allocator_buffer->size = alloc_step;              \
            }                                                               \
        }                                                                   \
                                                                            \
